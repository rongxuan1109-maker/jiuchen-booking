...
function dateRef() {
  ...
  const wd = isoWeekday(taipeiParts().date);
  const offMon = -(wd - 1);
  const names = ["一", "二", "三", "四", "五", "六", "日"];
  const rel: string[] = [];
  for (let i = 0; i < 7; i++) rel.push(`這週${names[i]}=${taipeiDatePlus(offMon + i)}`);
  for (let i = 0; i < 7; i++) rel.push(`下週${names[i]}=${taipeiDatePlus(offMon + 7 + i)}`);
  return out.join("、") + "。\n【週次對照】" + rel.join("、");
}
...
日期換算：「明天、後天、這週五、下週一」等一律對照下方【日期對照表】轉成 YYYY-MM-DD，**絕對不要自己心算星期或日期**(常算錯)。查到沒空檔就誠實說該天滿了、問要不要看別天。找不到預約就說「用這支電話沒找到預約耶，方便再確認一下號碼嗎」。
